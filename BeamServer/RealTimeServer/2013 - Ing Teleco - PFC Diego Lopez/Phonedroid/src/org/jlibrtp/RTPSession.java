...
public class RTPSession {
	 ...
	 //protected int pktBufBehavior = 1;
	 protected int pktBufBehavior = -1;
	 //protected int pktBufBehavior = 0;
		 ...
		 for(int i=0; i<buffers.length; i++) {
			 ...
			 if(this.mcSession) {
				 ...
				 while(iter.hasNext()) {			
					 ...
					 }catch (IOException ee){Log.e("unicast failed",ee.toString());} 
					 catch (Exception e) {
	...
	private void generateCNAME() {
		...
		if(this.mcSession) {
			...
			hostname = this.rtpSock.getLocalAddress().getHostAddress();//.getCanonicalHostName();//.getHostAddress();
		}
		...
		//cname = System.getProperty("user.name") + "@" + "0.0.0.0";
